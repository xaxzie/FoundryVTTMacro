(async () => {
    ...
    if (!actor) {
        ui.notifications.warn("⚠️ Sélectionnez le token d'Yunyun !");
        return;
    }

    // === CONFIGURATION ===

    // Custom Active Effects with Flags - Yunyun'S WATER MAGIC EFFECTS
    ...
    async function applyTokenTransformation(token, transformConfig, shouldTransform) {
        if (!token || typeof TokenMagic === "undefined") {
            console.warn("[Yunyun] Token Magic FX not available for transformation");
            return;
        }
        ...
        try {
            ...
            if (shouldTransform) {
                ...
                if (token.TMFXhasFilterId(filterId)) {
                    console.log(`[Yunyun] Token ${token.name} already has transformation ${filterId}`);
                    return;
                }
                ...
                console.log(`[Yunyun] Transformation applied successfully to ${token.name}`);

            } else {
                // Check if filter exists to revert
                if (!token.TMFXhasFilterId(filterId)) {
                    console.log(`[Yunyun] No transformation ${filterId} found on ${token.name} to revert`);
                    return;
                }
                ...
                setTimeout(async () => {
                    try {
                        if (token.TMFXhasFilterId(filterId)) {
                            ...
                            console.log(`[Yunyun] Deleted transformation filter ${filterId} from ${token.name}`);
                        }
                    } catch (error) {
                        console.error(`[Yunyun] Error deleting filter ${filterId}:`, error);
                    }
                }, loopDuration + 100); // Wait for animation duration + small buffer

                console.log(`[Yunyun] Reverting transformation on ${token.name}`);
                return; // Exit early since we're handling the deletion asynchronously
            }

        } catch (error) {
            console.error("[Yunyun] Error in token transformation:", error);
        }
    }
    ...
    async function playTransformationAnimation(token, animConfig, isActivating) {
        if (!token || typeof Sequence === "undefined") {
            console.warn("[Yunyun] Sequencer not available for transformation animation");
            return;
        }
        ...
        try {
            ...
            await seq.play();
            console.log(`[Yunyun] Transformation animation played`);
        } catch (error) {
            console.error(`[Yunyun] Error playing transformation animation:`, error);
        }
    }
    ...
    async function applyTokenFilters(token, filterConfig, shouldApply) {
        if (!token || typeof TokenMagic === "undefined") {
            console.warn("[Yunyun] Token Magic FX not available for filters");
            return;
        }
        ...
        try {
            ...
            if (shouldApply) {
                ...
                if (hasFilters) {
                    console.log(`[Yunyun] Token ${token.name} already has filters`);
                    return;
                }
                ...
                console.log(`[Yunyun] Applied ${filterConfigs.length} filter(s) to ${token.name}`);

            } else {
                // Remove filters
                const hasFilters = token.document.flags?.tokenmagic;
                if (!hasFilters) {
                    console.log(`[Yunyun] No filters found on ${token.name} to remove`);
                    return;
                }
                ...
                console.log(`[Yunyun] Removed filters from ${token.name}`);
            }

        } catch (error) {
            console.error("[Yunyun] Error in token filters:", error);
        }
    }
    ...
    async function playPersistentAnimation(token, animConfig, isActivating) {
        if (!token || typeof Sequence === "undefined") {
            console.warn("[Yunyun] Sequencer not available for persistent animation");
            return;
        }

        try {
            if (isActivating && animConfig.persistent) {
                console.log(`[Yunyun] Starting persistent water animation for ${token.name}`);
                ...
                if (offsetPixels !== 0) {
                    console.log(`[Yunyun] Applying Y offset of ${offsetPixels} pixels to ${animConfig.sequencerName}`);
                }
                ...
                await seq.play();
                console.log(`[Yunyun] Persistent water animation started: ${animConfig.sequencerName}`);

            } else if (!isActivating && animConfig.sequencerName) {
                console.log(`[Yunyun] Ending persistent water animation: ${animConfig.sequencerName}`);
                Sequencer.EffectManager.endEffects({ name: animConfig.sequencerName });
            }
        } catch (error) {
            console.error(`[Yunyun] Error with persistent animation:`, error);
        }
    }
    ...
    let dialogContent = `
        <h3>Gestionnaire d'Effets - Yunyun</h3>
    ...
    if (outsideEffects.length > 0) {
        dialogContent += `
            <div class="effect-section" style="border-color: #ff5722;">
                ...
                <p style="margin: 8px 0; font-size: 0.9em; color: #666;">Effets présents sur Yunyun mais non configurés dans ce gestionnaire</p>
        `;
        ...
        dialogContent += `
            <div class="effect-section" style="border-color: #2196f3;">
                ...
                <div class="effect-item" style="text-align: center; color: #666; font-style: italic;">
                    ...
                    <br><small>Les effets spécifiques de Yunyun seront ajoutés ici.</small>
                </div>
            </div>
        `;
    }
                    ...
                    if (action === 'removeExternal') {
                        ...
                        if (externalEffect) {
                            externalEffect.delete().then(() => {
                                ...
                                console.log(`[Yunyun] Removed external effect: ${externalEffect.name}`);
                            }).catch(error => {
                                console.error(`[Yunyun] Error removing external effect:`, error);
        ...
        try {
            ...
            if (canvas.tokens.controlled.length > 0) {
                ...
                for (const [effectKey, effectData] of Object.entries(CUSTOM_EFFECTS)) {
                    ...
                    if (existingEffect) {
                        console.log(`[Yunyun] Cleaning up visual effects for: ${effectData.name}`);
                        ...
                        if (effectData.hasTransformation) {
                            try {
                                ...
                                console.warn(`[Yunyun] Error removing transformation for ${effectData.name}:`, error);
                            }
                        }
                        ...
                        if (effectData.hasFilters) {
                            try {
                                ...
                                console.warn(`[Yunyun] Error removing filters for ${effectData.name}:`, error);
                            }
                        }
                        ...
                        if (effectData.hasAnimation && effectData.animation.persistent && effectData.animation.sequencerName) {
                            try {
                                ...
                                console.warn(`[Yunyun] Error ending animation for ${effectData.name}:`, error);
                            }
                        }
                    }
                }
                ...
                try {
                    if (typeof TokenMagic !== "undefined") {
                        ...
                        token.control({ releaseOthers: false });
                        await TokenMagic.deleteFiltersOnSelected();
                        console.log(`[Yunyun] Cleaned up all Token Magic FX filters`);
                    }
                } catch (error) {
                    console.warn(`[Yunyun] Error cleaning up all filters:`, error);
                }
                ...
                try {
                    if (typeof Sequencer !== "undefined") {
                        ...
                        console.log(`[Yunyun] Cleaned up all Sequencer effects`);
                    }
                } catch (error) {
                    console.warn(`[Yunyun] Error cleaning up all animations:`, error);
                }
            }
            ...
            console.error("[Yunyun] Error removing all effects:", error);
    ...
    try {
                ...
                if (newValue === 0 && currentCustomEffect) {
                    ...
                    if (customData.hasSpecialRemoval && customData.onRemoval) {
                        try {
                            ...
                            console.log(`[Yunyun] Executed special removal callback for increasable ${customData.name}`);
                        } catch (error) {
                            console.warn(`[Yunyun] Error in special removal callback for increasable ${customData.name}:`, error);
                        }
                    }

                    await currentCustomEffect.delete();
                    removedEffects.push(customData.name);
                    console.log(`[Yunyun] Removed increasable effect: ${customData.name}`);
                    ...
                    if (canvas.tokens.controlled.length > 0) {
                        ...
                        if (customData.hasTransformation) {
                            // Prevent double animation execution with a tempYunyunry flag
                            ...
                            if (customData.hasAnimation && customData.animation.deactivationFile) {
                                ...
                                if (window.YunyunAnimationLock && window.YunyunAnimationLock[animationKey]) {
                                    console.log(`[Yunyun] Deactivation animation already in progress for ${customData.name}, skipping`);
                                } else {
                                    try {
                                        // Set animation lock
                                        if (!window.YunyunAnimationLock) window.YunyunAnimationLock = {};
                                        window.YunyunAnimationLock[animationKey] = true;

                                        console.log(`[Yunyun] Playing TRANSFORMATION deactivation animation for ${customData.name}`);
                                        const seq = new Sequence()
                                        ...
                                        await seq.effect()
                                            ...
                                            .play();
                                        console.log(`[Yunyun] TRANSFORMATION deactivation animation completed for ${customData.name}`);

                                        // Clear lock after animation
                                        setTimeout(() => {
                                            if (window.YunyunAnimationLock) {
                                                delete window.YunyunAnimationLock[animationKey];
                                            }
                                        }, 3000);

                                        await new Promise(resolve => setTimeout(resolve, 200));
                                    } catch (error) {
                                        console.warn(`[Yunyun] Error playing deactivation animation: ${error}`);
                                        // Clear lock on error
                                        if (window.YunyunAnimationLock && window.YunyunAnimationLock[animationKey]) {
                                            delete window.YunyunAnimationLock[animationKey];
                                        }
                        ...
                        if (customData.hasAnimation && !customData.hasTransformation && !customData.hasFilters) {
                            ...
                            console.log(`[Yunyun] Cleaned up standalone animation for increasable ${customData.name}`);
                        }
                    }
                    ...
                    if (currentCustomEffect) {
                        ...
                        console.log(`[Yunyun] Updated increasable effect: ${customData.name} to ${newValue}`);
                    } else {
                        ...
                        console.log(`[Yunyun] Added increasable effect: ${customData.name} with ${newValue}`);
                        ...
                        if (canvas.tokens.controlled.length > 0) {
                            ...
                            if (customData.hasTransformation) {
                                ...
                                if (customData.hasAnimation && customData.animation.activationFile) {
                                    // Check animation lock
                                    if (window.YunyunAnimationLock && window.YunyunAnimationLock[animationKey]) {
                                        console.log(`[Yunyun] Animation already in progress for ${customData.name}, skipping`);
                                    } else {
                                        try {
                                            // Set animation lock
                                            if (!window.YunyunAnimationLock) window.YunyunAnimationLock = {};
                                            ...
                                            console.log(`[Yunyun] Playing TRANSFORMATION activation animation for ${customData.name}`);
                                            const seq = new Sequence()
                                            ...
                                            await seq.effect()
                                                ...
                                                .scale(customData.animation.scale || 0.8)
                                                .play();
                                            console.log(`[Yunyun] TRANSFORMATION activation animation completed for ${customData.name}`);
                                            ...
                                            console.warn(`[Yunyun] Error playing activation animation: ${error}`);
                                            // Clear lock on error
                                            if (window.YunyunAnimationLock && window.YunyunAnimationLock[animationKey]) {
                                                delete window.YunyunAnimationLock[animationKey];
                                            }
                            ...
                            if (customData.hasFilters) {
                                if (customData.hasAnimation) {
                                    if (customData.animation.persistent) {
                                        ...
                                        console.log(`[Yunyun] Skipping transformation animation for filter-based effect ${customData.name} (already played in transformation section)`);
                                    }
                                    await new Promise(resolve => setTimeout(resolve, 200));
                                }
                            ...
                            if (customData.hasAnimation && !customData.hasTransformation && !customData.hasFilters) {
                                ...
                                console.log(`[Yunyun] Played standalone animation for increasable ${customData.name}`);
                            }
                        }
                        ...
                        if (customData.onActivation) {
                            try {
                                ...
                                console.warn(`[Yunyun] Error in activation callback for ${customData.name}:`, error);
                            }
                        }
                    }
                }
        ...
        for (const [injuryKey, newValue] of Object.entries(injuryValues)) {
            ...
            if (newValue !== currentValue) {
                if (newValue === 0 && currentInjuryEffect) {
                    ...
                    console.log(`[Yunyun] Removed injury: ${injuryData.name || injuryData.label}`);
                } else if (newValue > 0) {
                    if (currentInjuryEffect) {
                        ...
                        console.log(`[Yunyun] Updated injury: ${injuryData.name || injuryData.label} to ${newValue}`);
                    } else {
                        ...
                        console.log(`[Yunyun] Added injury: ${injuryData.name || injuryData.label} with ${newValue}`);
                    }
                }
            }
        }
        ...
        for (const [effectKey, changeData] of Object.entries(changes)) {
            ...
            console.log(`[Yunyun] Processing: ${action} ${effectKey} in ${category}`);
            ...
            if (category === 'custom') {
                ...
                if (action === 'add') {
                    ...
                    console.log(`[Yunyun] Added effect: ${effectData.name}`);
                    ...
                    if (canvas.tokens.controlled.length > 0) {
                        ...
                        if (effectData.hasTransformation) {
                            ...
                            if (effectData.hasAnimation && effectData.animation.activationFile) {
                                ...
                                // Check animation lock
                                if (window.YunyunAnimationLock && window.YunyunAnimationLock[animationKey]) {
                                    console.log(`[Yunyun] Activation animation already in progress for ${effectData.name}, skipping`);
                                } else {
                                    try {
                                        // Set animation lock
                                        if (!window.YunyunAnimationLock) window.YunyunAnimationLock = {};
                                        ...
                                        console.log(`[Yunyun] Playing TRANSFORMATION activation animation for ${effectData.name}`);
                                        const seq = new Sequence();
                                        await seq.effect()
                                            ...
                                            .play();
                                        console.log(`[Yunyun] TRANSFORMATION activation animation completed for ${effectData.name}`);
                            ...
                            console.log(`[Yunyun] Applied transformation for ${effectData.name}`);
                        }
                        ...
                        if (effectData.hasFilters) {
                            ...
                            console.log(`[Yunyun] Applied filters for ${effectData.name}`);
                        }
                        ...
                        if (effectData.hasAnimation && !effectData.hasTransformation && !effectData.hasFilters) {
                            ...
                            console.log(`[Yunyun] Played standalone animation for ${effectData.name}`);
                        }
                    }
                    ...
                    if (effectData.onActivation) {
                        try {
                            ...
                            console.warn(`[Yunyun] Error in activation callback for ${effectData.name}:`, error);
                        }
                    }
                    ...
                    if (existing) {
                        ...
                        if (effectData.hasSpecialRemoval && effectData.onRemoval) {
                            try {
                                ...
                                console.log(`[Yunyun] Executed special removal callback for ${effectData.name}`);
                            } catch (error) {
                                console.warn(`[Yunyun] Error in special removal callback for ${effectData.name}:`, error);
                            }
                        }
                        ...
                        if (canvas.tokens.controlled.length > 0) {
                            ...
                            if (effectData.hasTransformation) {
                                ...
                                if (effectData.hasAnimation && effectData.animation.deactivationFile) {
                                    ...
                                    // Check animation lock
                                    if (window.YunyunAnimationLock && window.YunyunAnimationLock[animationKey]) {
                                        console.log(`[Yunyun] Deactivation animation already in progress for ${effectData.name}, skipping`);
                                    } else {
                                        try {
                                            // Set animation lock
                                            if (!window.YunyunAnimationLock) window.YunyunAnimationLock = {};
                                            ...
                                            console.log(`[Yunyun] Playing TRANSFORMATION deactivation animation for ${effectData.name}`);
                                            const seq = new Sequence();
                                            await seq.effect()
                                                ...
                                                .scale(effectData.animation.scale || 0.8)
                                                .play();
                                            console.log(`[Yunyun] TRANSFORMATION deactivation animation completed for ${effectData.name}`);
                                ...
                                await applyTokenTransformation(token, effectData.transformation, false);
                                console.log(`[Yunyun] Removed transformation for ${effectData.name}`);
                            }
                            ...
                            if (effectData.hasFilters) {
                                ...
                                await applyTokenFilters(token, effectData.filters, false);
                                console.log(`[Yunyun] Removed filters for ${effectData.name}`);
                            }
                            ...
                            if (effectData.hasAnimation && !effectData.hasTransformation && !effectData.hasFilters) {
                                ...
                                console.log(`[Yunyun] Cleaned up standalone animation for ${effectData.name}`);
                            }
                        }

                        await existing.delete();
                        removedEffects.push(effectData.name);
                        console.log(`[Yunyun] Removed effect: ${effectData.name}`);
                    }
                }
                ...
                if (action === 'setPosture') {
                    ...
                    if (postureData) {
                        ...
                        console.log(`[Yunyun] Added posture: ${postureData.name || postureData.label}`);
                    }
                    ...
                    console.log(`[Yunyun] Removed all postures`);
                }
                ...
                if (action === 'add') {
                    ...
                    console.log(`[Yunyun] Added status effect: ${statusData.name || statusData.label}`);
                    ...
                    if (existing) {
                        ...
                        console.log(`[Yunyun] Removed status effect: ${statusData.name || statusData.label}`);
                    }
                }
            }
        }
        ...
        console.error("[Yunyun] Error processing changes:", error);
