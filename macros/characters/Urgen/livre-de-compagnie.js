 ...
 * - Animation : projectile bleu depuis Urgen → cible, puis chauve-souris persistante sur la cible
 *
...
(async () => {
    ...
    const SPELL_CONFIG = {
        ...
        description: "Un livre s'envole et se lie à un allié, amplifiant ses dégâts.",
        manaCost: 2,
        isFocusable: false,
    ...
    try {
        ...
        ui.notifications.info("🎯 Sélectionnez la cible du Livre de Compagnie (allié ou vous-même)");
    ...
    function getActorAtLocation(targetX, targetY) {
        ...
        if (canvas.grid.type !== 0) {
            ...
            return { name: targetToken.name, token: targetToken, actor: targetToken.actor };
        } else {
            const tolerance = gridSize;
            const tokensAtLocation = canvas.tokens.placeables.filter(token => {
                const tokenCenterX = token.x + (token.w / 2);
                const tokenCenterY = token.y + (token.h / 2);
                const distance = Math.sqrt(Math.pow(targetX - tokenCenterX, 2) + Math.pow(targetY - tokenCenterY, 2));
    ...
    const existingEffect = targetActor.actor.effects.contents.find(e =>
    ...
    if (alreadyHasEffect) {
        ...
        console.log(`[DEBUG] Removed existing Livre de Compagnie from ${targetName}`);
    }
    ...
    async function playSpellAnimation() {
        try {
            ...
            // Persistent bat attachment animation
            sequence
                .effect()
    ...
    const result = await applyEffectWithGMDelegation(targetActor.token, effectData);
    ...
    if (result?.success !== false) {
        ...
        ui.notifications.info(`📖 Livre de Compagnie invoqué sur ${targetName} ! +${damageBonus} dégâts — ${SPELL_CONFIG.manaCost} mana`);
    } else {
        ui.notifications.error(`❌ Échec de l'application de l'effet sur ${targetName}.`);
    }

    console.log(`[DEBUG] Livre de Compagnie cast complete – Caster: ${actor.name}, Target: ${targetName}, Damage bonus: +${damageBonus}`);
})();
