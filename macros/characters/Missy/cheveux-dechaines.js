...
(async () => {
  ...
  const SPELL_CONFIG = {
    ...
    targeting: {
      color: "#9c27b0",
      texture:
        "modules/jb2a/Library/Generic/Marker/MarkerLight_01_Regular_Purple_400x400.webm",
    },
  };
  ...
  try {
    ...
    ui.notifications.info(
      "🎯 Sélectionnez la direction du cône de Cheveux Déchaînés...",
    );
    direction = await portal.pick();
  } catch (err) {
    ui.notifications.error(
      "❌ Erreur lors du ciblage. Vérifiez que le module Portal est installé.",
    );
    return;
  }
  ...
  const directionAngle = Math.atan2(
    direction.y - casterCenter.y,
    direction.x - casterCenter.x,
  );
  const directionDeg = ((directionAngle * 180) / Math.PI + 360) % 360;

  console.log(
    `[DEBUG] Cheveux Déchaînés direction: ${directionDeg.toFixed(1)}°`,
  );
  ...
  function findTargetsInCone() {
    ...
    for (const token of canvas.tokens.placeables) {
      ...
      if (angleDiff <= halfAngle) {
        ...
        console.log(
          `[DEBUG] Cone target: ${token.name}, dist=${(distPx / gridSize).toFixed(1)} cases, angleDiff=${angleDiff.toFixed(1)}°`,
        );
      }
    }
  ...
  if (coneTargets.length === 0) {
    ui.notifications.warn(
      "⚠️ Aucune cible trouvée dans le cône !\nLe sort est tout de même exécuté.",
    );
  }
      ...
      new Dialog(
        {
          ...
          buttons: {
            cast: {
              ...
              callback: (html) => {
                const checked = [
                  ...html.find('input[name="target"]:checked'),
                ].map((el) => parseInt(el.value));
                if (checked.length > SPELL_CONFIG.maxTargets) {
                  ui.notifications.warn(
                    `⚠️ Maximum ${SPELL_CONFIG.maxTargets} cibles !`,
                  );
                  return;
                }
                const bonusAttack =
                  parseInt(html.find("#bonusAttack").val()) || 0;
                resolve({
                  selectedTargets: checked.map((i) => coneTargets[i]),
                  bonusAttack,
                });
              },
            },
          ...
          render: (html) => {
            ...
            html.find('input[name="target"]').on("change", () => {
              const checkedCount = html.find(
                'input[name="target"]:checked',
              ).length;
              html
                .find("#target-warning")
                .toggle(checkedCount > SPELL_CONFIG.maxTargets);
  ...
  const attackDice =
    dexFull + getActiveEffectBonus(actor, SPELL_CONFIG.characteristic);
  const attackFormula = `${attackDice}d7 + ${levelBonus}${bonusAttack !== 0 ? ` + ${bonusAttack}` : ""}`;

  // Build combined roll with attack + all damage rolls
  let combinedRollParts = [attackFormula];

  // Add damage formulas for each target
  for (const assign of damageAssignments) {
    combinedRollParts.push(assign.formula);
  }
  ...
  // Extract damage results for each target
  for (let i = 0; i < damageAssignments.length; i++) {
    const assign = damageAssignments[i];
    const damageValue = combinedRoll.terms[0].results[i + 1].result;
  ...
  const manaCostDisplay =
    actualManaCost === 0 ? "GRATUIT" : `${actualManaCost} mana`;
  ...
  const noTargetsInZoneNote =
    coneTargets.length === 0
  ...
  // Send combined roll to chat (dice visible for attack + damage)
  ...
  // ===== NOTIFICATION =====
  const dmgSummary = rollResults
    .map((r) => `${r.target.name}: ${r.total}`)
    .join(", ");
  ui.notifications.info(
  ...
  console.log(
    `[DEBUG] Cheveux Déchaînés cast — Attack: ${attackRoll.total}, Targets: ${numTargets}, Results: ${dmgSummary}`,
  );
})();
