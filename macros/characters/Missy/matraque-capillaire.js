/**
 * Matraque Capillaire - Missy
 *
 * Missy utilise ses cheveux pour frapper un ennemi avec une masse capillaire dense et magique.
 *
 ...
 * - Animation : Fouet de cheveux avec teinte violette/magenta
 *
...
(async () => {
    ...
    const SPELL_CONFIG = {
        ...
        targeting: {
            range: 100, // Portée moyenne pour une attaque de cheveux
            color: "#9c27b0",
                ...
                content: `
                    <div style="padding: 15px; background: #f9f9f9; border-radius: 8px;">
                        ...
                        <div style="margin: 15px 0; padding: 10px; background: #f3e5f5; border-radius: 4px;">
                            <h4 style="margin-top: 0; color: #9c27b0;">💇‍♀️ Cheveux Magiques</h4>
                            <p style="margin: 5px 0; color: #333; font-size: 0.9em;">Attaque directe avec une masse de cheveux magiquement renforcés, basée sur la <strong>${SPELL_CONFIG.characteristicDisplay}</strong>.</p>
                            <p style="margin: 5px 0; color: #666; font-size: 0.8em;"><em>Les cheveux de Missy se densifient et frappent comme une matraque.</em></p>
                        </div>
                    </div>
                `,
                buttons: {
    ...
    async function playSpellAnimation() {
        try {
            // Play a hair whip animation from caster to target with purple tint
            await new Sequence()
                .effect()
                .file(SPELL_CONFIG.animations.whip)
                .attachTo(caster)
                .stretchTo(target)
    ...
    function createChatFlavor() {
        ...
        const damageDisplay = `
            <div style="text-align: center; margin: 8px 0; padding: 10px; background: #f3e5f5; border-radius: 4px;">
                <div style="font-size: 1.1em; color: #9c27b0; margin-bottom: 6px;"><strong>💇‍♀️ ${SPELL_CONFIG.name}${stanceNote}</strong></div>
                <div style="font-size: 0.9em; margin-bottom: 4px;"><strong>Cible:</strong> ${targetName}</div>
                <div style="font-size: 1.4em; color: #9c27b0; font-weight: bold;">💥 DÉGÂTS: ${finalDamageResult.total}</div>
    ...
    console.log(`[DEBUG] Matraque Capillaire cast complete - Caster: ${actor.name}, Target: ${targetName}, Damage: ${finalDamageResult.total}`);
