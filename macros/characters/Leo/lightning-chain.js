...
(async () => {
    ...
    function getActorAtLocation(x,y) {
        const tol = canvas.grid.size;
        const tokens = canvas.tokens.placeables.filter(token => {
            const cx = token.x + (token.document.width*canvas.grid.size)/2;
            const cy = token.y + (token.document.height*canvas.grid.size)/2;
        ...
        const act = tk.actor;
        if (!act) return null;
        const isOwner = act.isOwner; const isVisible = tk.visible; const isGM = game.user.isGM;
        return (isOwner||isVisible||isGM) ? { name: act.name, token: tk, actor: act } : { name: 'cible', token: tk, actor: act };
    }
