...
(async () => {
    ...
    async function applyTokenTransformation(token, transformConfig, shouldTransform) {
        if (!token || typeof TokenMagic === "undefined") {
            console.warn("[DEBUG] Aloha Effect: Token Magic FX not available for transformation");
            return;
        }
        ...
        try {
            ...
            if (shouldTransform) {
                ...
                setTimeout(async () => {
                    try {
                        ...
                        console.warn("[DEBUG] Aloha Effect: Error removing transformation filter:", error);
                    }
            ...
            console.error("[DEBUG] Aloha Effect: Error in token transformation:", error);
        }
    }
    ...
    async function playTransformationAnimation(token, animConfig, isActivating) {
        if (!token || typeof Sequence === "undefined") {
            console.warn("[DEBUG] Aloha Effect: Sequencer not available for animation");
            return;
        }
        ...
        try {
            ...
            console.log(`[DEBUG] Aloha Effect: Played transformation animation for ${token.name}`);

        } catch (error) {
            console.error("[DEBUG] Aloha Effect: Error in transformation animation:", error);
        }
    }
    ...
    async function applyTokenFilters(token, filterConfig, shouldApply) {
        if (!token || typeof TokenMagic === "undefined") {
            console.warn("[DEBUG] Aloha Effect: Token Magic FX not available for filters");
            return;
        }
        ...
        try {
            ...
            if (shouldApply) {
                ...
                const hasFilters = token.document.flags?.tokenmagic;
                if (!hasFilters) {
                    console.log("[DEBUG] Aloha Effect: No filters to remove");
                    return;
                }
            ...
            console.error("[DEBUG] Aloha Effect: Error in token filters:", error);
        }
    }
    ...
    async function playPersistentAnimation(token, animConfig, isActivating) {
        if (!token || typeof Sequence === "undefined") {
            console.warn("[DEBUG] Aloha Effect: Sequencer not available for persistent animation");
            return;
        }
        ...
        try {
            ...
            console.error("[DEBUG] Aloha Effect: Error in persistent animation:", error);
        }
    }
