/**
 * Cuisine à Blanc - Aloha
 *
 * Nom: Cuisine à Blanc
 * Coût : 3 mana (focalisable)
 * Niveau : 1
 * Description : Aloha chauffe sa poêle à blanc et porte un coup dévastateur avec une chaleur intense.
 * Toucher : Physique (+ effets actifs + bonus manuels d'attaque)
 * Dégâts : 1d6 + Physique + bonus manuels + bonus d'effets actifs
 *
...
(async () => {
            ...
            const html = `
                <div>
                    ...
                    <div style="margin-top:10px; padding:8px; border:1px solid #ff5722; border-radius:6px; background:#fff3e0;">
                        <p><strong>🔥 Description :</strong> Aloha chauffe sa poêle à blanc, créant une chaleur intense qui augmente considérablement les dégâts de son attaque.</p>
                        <p><strong>⚡ Puissance :</strong> La poêle chauffée à blanc inflige des dégâts thermiques dévastateurs (1d6 + Physique).</p>
                        <p><strong>🍳 Technique :</strong> Sort de niveau 1 nécessitant plus de concentration et d'énergie magique.</p>
                    </div>
                </div>
            `;
    ...
    // Play intense heated pan animation
    try {
        if (typeof Sequence !== 'undefined') {
            const seq = new Sequence();

            // Heat wave effect at caster location first
            if (SPELL.animations.heatWave) {
                seq.effect()
    ...
    async function calculateDamage() {
        ...
        const totalDamageBonus = charInfo.final + (damageBonus || 0) + effectDamageBonus;
    ...
    const levelBonus = 2 * SPELL.spellLevel; // level 1 -> 2
    ...
    function createChatFlavor() {
        ...
        const physiqueInfo = `
            <div style="color: #ff5722; font-size: 0.9em; margin: 5px 0;">
                <div>🍳 Bonus Physique Complet : +${charInfo.final} dégâts</div>
            </div>
        `;
        ...
        const damageDisplay = `
            <div style="text-align: center; margin: 8px 0; padding: 10px; background: linear-gradient(135deg, #fff3e0, #f3e5f5); border-radius: 4px;">
                <div style="font-size: 1.1em; color: #e65100; margin-bottom: 6px;"><strong>🔥 ${SPELL.name}${stanceNote}</strong></div>
                <div style="font-size: 0.9em; margin-bottom: 4px;"><strong>Cible:</strong> ${targetName}</div>
                <div style="font-size: 1.4em; color: #d84315; font-weight: bold;">💥 DÉGÂTS: ${finalDamageResult.total}</div>
                <div style="font-size: 0.8em; color: #666; margin-top: 2px;">
                    (${SPELL.baseDamageFormula} + ${SPELL.characteristicDisplay} + bonus)
                </div>
                <div style="font-size: 0.8em; color: #666;">
                    Poêle chauffée à blanc - Dégâts thermiques intenses
                </div>
            </div>
        `;

        return `
            <div style="background: linear-gradient(135deg, #fff3e0, #f3e5f5); padding: 12px; border-radius: 8px; border: 2px solid #e65100; margin: 8px 0;">
                <div style="text-align: center; margin-bottom: 8px;">
                    <h3 style="margin: 0; color: #e65100;">🔥 ${SPELL.name}</h3>
                    <div style="margin-top: 3px; font-size: 0.9em;">
                        <strong>Chef:</strong> ${actor.name} | <strong>Coût:</strong> ${actualManaCostDisplay}
    ...
    ui.notifications.info(`🔥 ${SPELL.name} lancé !${stanceInfo} Cible: ${targetName}. Attaque: ${attackResult.result}, Dégâts: ${finalDamageResult.total}${maximizedInfo}. Poêle chauffée à blanc !`);
