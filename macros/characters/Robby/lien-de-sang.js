...
(async () => {
  ...
  function findAllyOnCanvas(actorId) {
    const allyToken = canvas.tokens.placeables.find(
      (t) => t.actor?.id === actorId && t.document.disposition !== -1, // Exclude hostile tokens (just in case)
    );
  ...
  if (Object.keys(detectedAllies).length === 0) {
    ui.notifications.warn(
      "⚠️ Aucun allié de Robby n'est présent sur le champ de bataille !",
    );
    return;
  }
  ...
  function hasBondEffect(targetActor) {
    return targetActor.effects.contents.some(
      (e) => e.flags?.world?.lieSangRobby === true,
    );
  }

  function getBondEffect(targetActor) {
    return targetActor.effects.contents.find(
      (e) => e.flags?.world?.lieSangRobby === true,
    );
  }

  // ===== BUILD DIALOG =====
  const allyRows = Object.entries(detectedAllies)
    .map(([name, { actor: allyActor, token }]) => {
          ...
          confirm: {
            ...
            callback: (html) => {
              ...
              for (const name of Object.keys(detectedAllies)) {
                const val = html
                  .find(`input[name="ally-${name}"]:checked`)
                  .val();
  ...
  async function applyEffectToAlly(targetActor, targetToken) {
    ...
    if (targetActor.isOwner) {
      ...
      ui.notifications.warn(
        `⚠️ Impossible d'appliquer l'effet sur ${targetActor.name} – droits insuffisants.`,
      );
    }
  }
  ...
  async function removeEffectFromAlly(targetActor, targetToken) {
    ...
    if (targetActor.isOwner) {
      ...
      ui.notifications.warn(
        `⚠️ Impossible de retirer l'effet sur ${targetActor.name} – droits insuffisants.`,
      );
    }
  }
  ...
  const currentBondList =
    allCurrentlyBound.length > 0
  ...
  if (deactivated.length > 0)
    parts.push(`✂️ Retiré : ${deactivated.join(", ")}`);
  if (parts.length === 0) {
  ...
  console.log(
    `[DEBUG] Lien de Sang complete – Activated: ${activated.join(", ")} | Deactivated: ${deactivated.join(", ")}`,
  );
})();
